  ...
  var setupControls = function ($box, settings) {
    ...
    if (settings.next != null) {
      $controls = $controls.add($(settings.next).on(
        'swipeDown', { reverse: false }, nextSlideListener
      ));
    ...
    if (settings.prev != null) {
      $controls = $controls.add($(settings.prev).on(
        'swipeUp', { reverse: true }, nextSlideListener
      ));

    }
      if (settings.next1 != null) {
          $controls = $controls.add($(settings.next1).on(
              'tap', { reverse: false }, nextSlideListener
          ));
      }

      if (settings.prev1 != null) {
          $controls = $controls.add($(settings.prev1).on(
              'tap', { reverse: true }, nextSlideListener
          ));
      }
      if (settings.next2 != null) {
          $controls = $controls.add($(settings.next2).on(
              'swipeLeft', { reverse: false }, nextSlideListener
          ));
      }

      if (settings.prev2 != null) {
          $controls = $controls.add($(settings.prev2).on(
              'swipeRight', { reverse: true }, nextSlideListener
          ));
      }
      if (settings.next3 != null) {
          $controls = $controls.add($(settings.next3).on(
              'swipeLeft', { reverse: false }, nextSlideListener
          ));
      }
      if (settings.next5 != null) {
          $controls = $controls.add($(settings.next5).on(
              'swipeLeft', { reverse: false }, nextSlideListener
          ));
      }
      if (settings.next4 != null) {
          $controls = $controls.add($(settings.next4).on(
              'swipeLeft', { reverse: false }, nextSlideListener
          ));
      }
      if (settings.prev3 != null) {
          $controls = $controls.add($(settings.prev3).on(
              'swipeRight', { reverse: true }, nextSlideListener
          ));
      }
    if (settings.pause != null) {
      $controls = $controls.add($(settings.pause).on(
        'tap', playPauseListener
      ));
    }
